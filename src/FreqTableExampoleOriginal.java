...
public class FreqTableExampoleOriginal {

    public static final int NUM_ASCII_CHAR=128;

    public static void main(String[] args){
        int[] freqs= createFreqTableURL("http://www.naver.com");
        ...
        if( freqs.length==0 )
            ...
            for( int i=0; i<NUM_ASCII_CHAR; i++){
                System.out.println("character code: "+i+" ,character: "+(char)i+" ,frequency: "+freqs[i]);
            }
        ...
        try{
            ...
            System.out.println("Problem while reading from file. Unable to create freq table"+2);
        }

        if( freqs.length==0 ){
            System.out.println("No frequency table created due to nproblems when reading from file");
        }else{
            for( int i =0; i<freqs.length;i++){
                System.out.println("charater code: "+i+" ,charater: "+(char)i+" ,frequecy: "+freqs[i]);
            }
            System.out.println("Total charaterset in file: "+sum(freqs));
        }
    }

    private static int sum(int[] list){
        assert list !=null : "Failed precondition, sum: parameter list"+" may noit be null";
        int total =0;
        for(int x :list){
            total += x;
        }
